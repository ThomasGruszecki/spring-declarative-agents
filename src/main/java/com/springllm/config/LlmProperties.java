 ...
 * Maps properties under the 'llm' prefix in application.yml/properties.
 * Example:
 * llm:
 * OpenAI:
 * api-key: sk-xxxxxxxx
...
public class LlmProperties {
    ...
    /**
     * A map where the key is the provider name (e.g., "OpenAI", "Claude")
     * and the value contains the specific configuration for that provider.
     */
    ...
    public ProviderConfig getProvider(String providerName) {
        ...
        if (config == null) {
            throw new IllegalArgumentException("Configuration for LLM provider '" + providerName + "' not found under 'llm' properties.");
        }
        return config;
    }
}
